...
{
	MajorVersion = OTB_VERSION_2;
	MinorVersion = CLIENT_VERSION_800;
	...
	for(pugi::xml_node itemAttributesNode = itemNode.first_child(); itemAttributesNode; itemAttributesNode = itemAttributesNode.next_sibling()) {
		...
		if (key == "type") {
			...
			to_lower_str(key);
			if (typeValue == "ground") {
				it.group = ITEM_GROUP_GROUND;
			} else if (typeValue == "container") {
				it.group = ITEM_GROUP_CONTAINER;
			} else if (typeValue == "charges") {
				it.group = ITEM_GROUP_RUNE;
				it.client_chargeable = true;
			} else if (typeValue == "teleport") {
				it.group = ITEM_GROUP_TELEPORT;
			} else if (typeValue == "magicfield") {
				it.group = ITEM_GROUP_MAGICFIELD;
			} else if (typeValue == "writeable") {
				it.group = ITEM_GROUP_WRITEABLE;
			} else if (typeValue == "splash") {
				it.group = ITEM_GROUP_SPLASH;
			} else if (typeValue == "fluid") {
			...
			it.unpassable = itemAttributesNode.attribute("value").as_bool();
		} else if(key == "clientid") {
			it.clientID = itemAttributesNode.attribute("value").as_int();
			it.sprite = static_cast<GameSprite*>(g_gui.gfx.getSprite(it.clientID));
		} else if (key== "hasheight") {
			it.hasElevation = itemAttributesNode.attribute("value").as_bool();
		} else if (key== "alwaysontop") {
			it.alwaysOnBottom = itemAttributesNode.attribute("value").as_bool();
		} else if (key== "alwaysontoporder") {
			it.alwaysOnTopOrder = itemAttributesNode.attribute("value").as_int();
		} else if (key== "isvertical") {
			it.hookEast= itemAttributesNode.attribute("value").as_bool();
		} else if (key == "pickupable") {
			it.pickupable = itemAttributesNode.attribute("value").as_bool();
		} else if (key== "ishorizontal") {
			it.hookSouth = itemAttributesNode.attribute("value").as_bool();
		} else if (key== "ishangable") {
			it.isHangable = itemAttributesNode.attribute("value").as_bool();
		} else if (key== "stackable") {
			it.stackable = itemAttributesNode.attribute("value").as_bool();
		} else if (key== "rotatable") {
			it.rotable = itemAttributesNode.attribute("value").as_bool();
